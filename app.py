# SafeHarbor Bot v57
# שינויים מ-v56:
#   - בסוף תרגיל הקרקוע (step 6): ניתוח תגובת המשתמש
#     חיובי → MSG_GROUNDING_POSITIVE (יופי שעצרת...)
#     היסוס / שלילי → MSG_RETURNING (היי, טוב שחזרת...)
...
GROUNDING_STEPS = [
    ...
    "\U0001f499 איך התחושה עכשיו?"   # ← step 5: שאלת סיום
]
...
# v57: מילות תגובה חיובית לשאלת סיום קרקוע
...
# v57: זיהוי תגובה חיובית לשאלת סיום קרקוע
...
def is_grounding_positive(text):
    """
    v57: מזהה תגובה חיובית לשאלת "איך התחושה עכשיו?"
    בדיקה כפולה: קודם שלילי (מנצח), אחר כך חיובי.
    ספק → שלילי (→ MSG_RETURNING, יותר בטוח לבריאות הנפש).
    """
    if _GROUNDING_NEGATIVE_RE.search(text):
        return False
    return bool(_GROUNDING_POSITIVE_RE.search(text))
...
def _handle_message_inner(phone, text):
    ...
    if tool == "grounding":
        ...
        # ── v57: step 5 = שאלת סיום "איך התחושה עכשיו?" ──
        # המשתמש ענה → נתח תגובה
...
def root():
    return "SafeHarbor Bot is running v57", 200
